...
export async function POST(req:NextRequest){
 ...
 try{
  ...
  const jobs=(d.results||[]).map((j:RawJob)=>({id:j.id,title:j.title||'Untitled role',company:j.company?.display_name||'Employer',location:j.location?.display_name||'United States',url:j.redirect_url,description:j.description||'',salaryMin:j.salary_min??null,salaryMax:j.salary_max??null,created:j.created||null,employmentType:j.contract_time||j.contract_type||null,matchScore:scoreJob(j.description||'',resume),titleRelevance:titleScore(j.title||'',title)})).sort((a:any,b:any)=>resume.trim()?(b.matchScore??-1)-(a.matchScore??-1)||b.titleRelevance-a.titleRelevance:b.titleRelevance-a.titleRelevance);
